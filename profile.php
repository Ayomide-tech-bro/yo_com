    ...
    if (isset($_FILES['user_dp'])) { 
        ...
        if (in_array($file_type, $supported_types)) { 
            if ($file_size<=2000000) {
                $upload = move_uploaded_file($file_tmp_name, 'user\uploads'.$new_file_name);
                if ($upload) { 
                    ...
                    if ($n_row2 > 0) { 
                        if (is_writable('user\uploads\user_dp'.$old_file_name)&&($old_file_name!='')) {
                            unlink('user/uploads/user_dp/'.$old_file_name);
                        }
